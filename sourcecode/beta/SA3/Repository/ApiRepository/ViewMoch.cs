...
public partial class ViewMoch
{
	...
	/// <remarks />
	[NotMapped]
	public string CsvValue => ToCsvValue();
	...
	/// <remarks/>
	private string CheckEmail() { if (Email1.ToLower().Contains("haderslev.dk")) return Email1; else if (Email2.ToLower().Contains("haderslev.dk")) return Email2;
		else if (Email1.ToLower().Contains("udcit.dk")) return Email1; else if (Email2.ToLower().Contains("udcit.dk")) return Email2; else if (Email1.ToLower().Contains("skolekom.dk")) return Email1;
		else if (Email2.ToLower().Contains("skolekom.dk")) return Email2; else if (!string.IsNullOrEmpty(Email1)&&!Email1.Equals("Empty@Empty.Com")) return Email1;
		else if (!string.IsNullOrEmpty(Email2)&&!Email2.Equals("Empty@Empty.Com")) return Email2; else return "Empty@Empty.Com"; }

	/// <returns>this as CSV value string</returns>
	private string ToCsvValue() { string email = CheckEmail(); return "\""+email+"\",\""+Fornavn+" "+Efternavn+"\",\""+email+"\",\"/Root/MOCH/Haderslev Kommune/Haderslev Kommune/"+Afdeling+ ":User\",\"Haderslev\",\"Haderslev123\",\"true\"\r\n"; }
