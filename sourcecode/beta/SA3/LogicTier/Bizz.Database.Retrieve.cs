...
public partial class Bizz // Database Retrieve
{
	...
	protected Institution RetrieveInstitution(string institutionId) { if (!institutionId.IsNullOrWhiteSpace()&&!institutionId.Equals("NO")) { Dictionary<string,Institution> dict=GetInstitutionDict();
		if (dict.ContainsKey(institutionId)) return dict[institutionId]; } return new(); }
