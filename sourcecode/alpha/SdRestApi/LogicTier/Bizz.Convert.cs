...
public partial class Bizz // Convert
{
	...
	private string ConvertApiEntityListToCsvString<T>(List<T> list) where T : class { string result=""; bool headerReady=false; foreach (T obj in list) { if (!headerReady) { switch (typeof(T).Name) {
		...
		switch (typeof(T).Name) { case "View3in1Organization": result += (obj as View3in1Organization).CsvValue; break; case "View3in1OrganizationStructure": result += (obj as View3in1OrganizationStructure).CsvValue; break;
			...
			case "ViewKantine": result += (obj as ViewKantine).CsvValue; break; case "ViewMoch": result += (obj as ViewMoch).ToCsvValue(this.Config.Roles,this.Config.PassWord); break;
	...
	private string ConvertApiEntityListToXmlString<T>(List<T> list) where T : class { string result = "\u003C\u003Fxml version=\u00221\u002E0\u0022 encoding=\u0022utf-16\u0022\u003F\u003E"+Environment.NewLine;
		string type = typeof(T).Name; result+="\u003C"+type+"s\u003E" + Environment.NewLine; foreach (T obj in list) { switch (type) { case "View3in1Organization": result += (obj as View3in1Organization).ToXmlString(); break;
			...
			case "ViewMoch": result += (obj as ViewMoch).ToXmlString(this.Config.Roles,this.Config.PassWord); break; case "ViewOrganization": result += (obj as ViewOrganization).ToXmlString(); break;
