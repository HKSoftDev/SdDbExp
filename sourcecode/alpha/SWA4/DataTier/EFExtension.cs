...
public class EFExtension
{
	...
	public static List<T> SelectViewFromDatabase<T>() where T : class { using EFContext dbo=new(); return typeof(T).Name.ToLower() switch {
		...
		"viewinstitution" => dbo.ViewInstitutionList.FromSqlRaw("SELECT * FROM [SD].[dbo].[ViewInstitutionList]").ToListAsync().Result as List<T>,
		"viewkantine" => dbo.ViewKantineList.FromSqlRaw("SELECT * FROM [SD].[dbo].[ViewKantineList] ORDER BY Cpr,Tjenestenummer,Afdelingkode,Beskæftigelsesdecimal DESC").ToListAsync().Result as List<T>,
		"viewmoch" => dbo.ViewMochs.FromSqlRaw("SELECT * FROM [SD].[dbo].[ViewMochs] ORDER BY [Afdeling],[Tjenestenummer],[Efternavn],[Fornavn]").ToListAsync().Result as List<T>,
		"vieworganization" => dbo.ViewOrganizationList.FromSqlRaw("SELECT * FROM [SD].[dbo].[ViewOrganizationList]").ToListAsync().Result as List<T>,
		"vieworganizationstructure" => dbo.ViewOrganizationStructureList.FromSqlRaw("SELECT * FROM [SD].[dbo].[ViewOrganizationStructureList]").ToListAsync().Result as List<T>,
